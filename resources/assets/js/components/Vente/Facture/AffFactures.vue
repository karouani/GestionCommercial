...
import  Pagination from '../../Pagination.vue';
  ...
  var today = new Date(this.currentDate);
   ...
   'test1.searchQuery':
    ...
    handleOk(){
        this.$router.push({ name: 'addFacture', params: { id_compte: this.compte.id_compte ,reference_f: this.facture.reference_f ,currentDate: this.currentDate }});
...
countFactures(){
          ...
          getFactures(){//type_status
                axios.get('/getFactures?page='+this.factures.current_page+'',{params: { type_operation_f: 'vente',anneeF:this.anneeF } })
                .then((response) => {
                    this.loading = false;
             ...
             if(this.search === ""){
                //console.log('test2');
                    this.getFactures();}
                     ...
                     // console.log('test1');
                axios.get('/searchFactures/'+this.search+'?page='+this.factures.current_page+'',{params: { type_operation_f: 'vente',anneeF:this.anneeF } })
                .then((response) => {
