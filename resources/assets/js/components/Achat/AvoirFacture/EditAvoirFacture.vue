...
<div v-if="!loading">
     <div class="row">
        <div class="col">
        <router-link class="btn btn-primary mb-3 retour  float-right " :to="'/getAvoirFacturesA'"> <i class="fas fa-long-arrow-alt-left fontsize"></i> </router-link>
        </div>
    </div>
 ...
 updateAvoirFacture: function(){
     ...
     console.log(this.avoirFacture.date_limit_af)
        ...
        axios.post('/updateAvoirFacture',{avoirFactures:this.avoirFacture,commandes:this.commandes,modePaiements:this.modePaiement,suppCommandes: this.suppCommandes}).then( response => {             
                    this.$router.push('/getAvoirFacturesA/edit');  
...
computed:{
   ...
   diff(){
            ...
            created () {
         if(this.$route.params.fk_compte_af== undefined){
