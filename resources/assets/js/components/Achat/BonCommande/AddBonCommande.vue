...
<div>
    ...
    <div class="row">
        <div class="col">
        <router-link class="btn btn-primary mb-3 retour float-right " :to="'/ShowBonCommandesA'"> <i class="fas fa-long-arrow-alt-left fontsize"></i> </router-link>
        </div>
    </div>     
...
<div class="row">
    <div class="col">
        <br>
    <h5><i class="far fa-file"></i> Ajouter un bon de commande : <strong>{{bonCommande.reference_bc}}</strong></h5>
    </div>
</div>
<hr>
<div class="row">
  ...
  return Math.round(number * factor) / factor;
    ...
    addBonCommande(){
        ...
        .then(response => {         
               ...
               // this.$router.push('/');
               this.$router.push({ name: 'ShowBonCommandesA', params: { success: "addsuccess"  }});
        })
        .catch(() => {
...
commandes: {
      ...
      'echeance':{
         ...
         this.bonCommande.reference_bc = 'BCA-'+referenceBC[1]+'-'+referenceBC[2];
                 this.echeancee;
                    this.diff;
