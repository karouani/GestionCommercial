...
<div v-if="!loading">
  <div class="text-center pull-right" >
    ...
    <div class="row" >
            ...
            <input type="number" @keyup.enter="getFactures"  class="form-control" v-model="anneeF" placeholder="recherche par année " aria-label="Username" aria-describedby="basic-addon1" >
            </div>  
                                </div> 
  <div class="col"  >
...
import  Pagination from '../../Pagination.vue';
    ...
    export default{
      ...
      mounted(){
                 this.anneeF = new Date().getFullYear();
  ...
  var today = new Date(this.currentDate);
                    var yyyy = today.getFullYear();             
                    this.year  = yyyy;
                     this.facture.reference_f='FA-'+this.year+'-'+this.nbrFactures;
        ...
        this.$router.push({ name: 'addFactureA', params: { id_compte:  this.compte.id_compte ,fk_compagnie:this.compte.fk_compagnie ,reference_f: this.facture.reference_f ,currentDate: this.currentDate }});
            ...
            redirect_To_EditFacture(facture){
                   //  this.$router.push('/ShowBonCommande/'+reference_bc);
                     this.$router.push({ name: 'EditFactureA', params: {id_facture: facture.id_facture, reference_f: facture.reference_f, fk_compte_f: facture.fk_compte_f}});

            },
               redirect_To_FactureDetails(facture){
                    this.$router.push({ name: 'FactureDetailsA', params: {id_facture: facture.id_facture, reference_f: facture.reference_f}});
...
countFactures(){
                ...
                .then((response) => {
                   ...
                   var today = new Date();
                    ...
                    this.facture.reference_f='FA-'+year+'-'+this.nbrFactures;
          ...
          getFactures(){//type_status
                axios.get('/getFactures?page='+this.factures.current_page+'',{params: { type_operation_f: 'achat',anneeF:this.anneeF } })
                .then((response) => {
                    this.loading = false;
             ...
             if(this.search === ""){
                //console.log('test2');
                    this.getFactures();}
                     ...
                     // console.log('test1');
                axios.get('/searchFactures/'+this.search+'?page='+this.factures.current_page+'',{params: { type_operation_f: 'achat',anneeF:this.anneeF } })
                .then((response) => {
